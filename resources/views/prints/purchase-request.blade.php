...
<body>
@php
    $documentDate = $documentDate ? \Illuminate\Support\Carbon::parse($documentDate) : now();
    $groupedItems = $items->groupBy(fn ($item) => $item->category_name ?: 'Items');
    $grandTotal = $items->sum(fn ($item) => (float) $item->estimated_budget);
    $officeSection = \Illuminate\Support\Str::upper($departmentName ?: '____________________________');
    $fundCluster = $fundSourceName ?: '________________';
    $responsibilityCode = $responsibilityCenterCode ?: '________________';
    $requesterPrintedName = $requesterName ? \Illuminate\Support\Str::upper($requesterName) : '________________';
    $approverPrintedName = $approverName ? \Illuminate\Support\Str::upper($approverName) : '________________';
    $rowNumber = 1;
@endphp
...
<div class="pr-sheet">
    ...
    <table class="form-table">
        <tr>
            <td rowspan="2" style="width: 34%;">
                ...
                <div class="office-value">{{ $officeSection }}</div>
            </td>
            <td style="width: 28%;">
        ...
        <tr>
            <td colspan="2">
                ...
                <div class="field-value">{{ $responsibilityCode }}</div>
            </td>
            <td>
                <span class="field-label">Fund Source:</span>
                <div class="field-value">{{ $fundSourceName ?: '________________' }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="items-table">
        <thead>
            <tr>
                <th class="center" style="width: 9%;">Stock/<br>Property No.</th>
        ...
        <tbody>
            @forelse($groupedItems as $categoryName => $categoryItems)
                <tr class="category-row">
                    <td colspan="6">{{ \Illuminate\Support\Str::upper($categoryName) }}</td>
                </tr>
    ...
    <table class="purpose-table">
        <tr>
            ...
            <td class="purpose-value">{{ $purchaseRequest->purpose ?: '________________' }}</td>
        </tr>
    </table>
        ...
        <tr>
            <td>
                ...
                <div class="signature-entry">
                    ...
                    <span class="signature-text strong">{{ $requesterPrintedName }}</span>
                </div>
                <div class="signature-entry">
                    <span class="signature-label">Designation:</span>
                    <span class="signature-text">{{ $requesterDesignation ?: '________________' }}</span>
                </div>
            </td>
            <td>
                ...
                <div class="signature-entry">
                    <span class="signature-label">Printed Name:</span>
                    <span class="signature-text strong">{{ $approverPrintedName }}</span>
                </div>
                <div class="signature-entry">
                    <span class="signature-label">Designation:</span>
                    <span class="signature-text">{{ $approverDesignation ?: '________________' }}</span>
                </div>
            </td>
        </tr>
    </table>
